    ...
    def _parse_at(self, text: str, pos: int) -> tuple[list, int]:
        ...
        while pos < len(text):
            ...
            if char == "[":
                ...
                if node.orelse:
                    else_line = self._find_keyword_line( document, line_offsets, node.handlers[-1].end_lineno if node.handlers else node.body[-1].end_lineno, node.orelse[0].lineno)
                if not isinstance(
                    node,
...
batch_request_schema = {
    "properties": {
        "identities": _request_identities_schema | {
            "description": _request_identities_schema[
                'description'
            ] + _request_level_description
        },
        "batch": {
            "type": "array",
...
my_set = {1}
my_set2 = {1, 2}
long_set = {"hello_there", "how_are_you", "doing_today"}
nested_set = [{"alpha", "beta"}, {"gamma", "delta"}]
frozen = frozenset({"thing_one", "thing_two", "thing_three"})



while condition_a is True or condition_b is True or condition_c is True:
    print("looping")

while x < 10 and y < 20:
    x += 1

while some_long_variable_name is True and another_long_variable_name is True and yet_another_variable is True:
    break

while counter + offset + padding + margin > maximum_allowed_value:
    counter -= 1



should_run = x > 0 and y > 0
should_stop = error_count > max_errors or timeout_reached is True or user_cancelled is True
is_valid = has_name is True and has_email is True and (is_admin is True or has_permission is True)
...
@decorator_one
@decorator_two
@decorator_three(param="value", other="thing", more="stuff")
def multi_decorated():
    pass
...
first, *rest = my_list
a, b, *remaining = get_values(source="database", timeout=30, retries=3)
merged = {**defaults, **overrides, "extra": "value"}
result = my_func(*args, **kwargs)
combined = [*list_one, *list_two, "extra"]
...
try:
    ...
    cleanup()
try:
    data = fetch_data(url="https://example.com", timeout=30, headers={"Authorization": "Bearer token"})
...
with database.transaction(isolation="serializable", timeout=30, retries=3) as txn:
...
def positional_only(x: int, y: int, /, z: int=0) -> int:
    return x + y + z
def keyword_only(*, name: str, value: int, default: bool=False) -> dict:
    return {"name": name, "value": value}
def mixed_params(pos_only: int, /, normal: str, *args, kw_only: bool=True, **kwargs) -> None:
    pass
def short_star(x, /, y, *, z):
    return x + y + z
...
offset = calculate_base(x=start_pos, y=end_pos) + calculate_adjustment(factor=scale, offset=margin)
...
mixed_expand(first_arg, second_arg, keyword_one="value_one", keyword_two="value_two")
all_kwargs_expand(alpha="first", beta="second", gamma="third")
...
nested_in_if = isinstance(node, ast.Call) and len(node.args) > 0 and any(isinstance(a, ast.Starred) for a in node.args)

long_method_chain = queryset.filter(active=True).exclude(deleted=True).order_by("-created").select_related("author")

dict_with_calls = {"computed": compute_value(input_data, transform="normalize"), "static": "hello", "nested": {"inner": get_inner(key="test")}}

multiline_return_dict = {"status": "success", "data": process_response(raw_data, format="json", validate=True), "metadata": {"timestamp": now(), "version": "1.0"}}


result = my_func(arg1, arg2, kwarg1="value1", kwarg2="value2") + other_func(x, y, z, key="thing") == expected_val

if (some_condition is True or other_condition is True) and validate(input_data, schema={"type": "object", "required": ["name", "age"]}) is True:
    print("valid")

config = {"handlers": [create_handler(name="stdout", level="DEBUG", formatter=build_formatter(style="json", indent=4))], "loggers": {"root": {"level": "INFO", "handlers": ["stdout"]}}}

response = client.post("/api/v1/users", json={"name": "test", "email": "test@example.com", "roles": ["admin", "user"]}, headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"})

pipeline = data.filter(lambda x: x > 0).map(lambda x: x * 2).reduce(lambda acc, x: acc + x, 0)
...
def process_batch(items: list[dict[str, str | int | None]], config: dict[str, dict[str, list[str]]] | None=None, callback: Callable[[dict], bool] | None=None) -> dict[str, list[dict[str, Any]]]:
    pass
...
result = (func_a(x=1, y=2, z=3) if condition_alpha and condition_beta else func_b(a="hello", b="world", c="test"))

while not queue.empty() and (time.time() - start_time < timeout or force_continue is True):
    item = queue.get()

try:
    result = await asyncio.gather(*[process_item(item, config={"timeout": 30, "retries": 3}) for item in batch])
except (asyncio.TimeoutError, ConnectionError) as exc:
    log_error(message=f"Batch failed: {exc}", context={"batch_size": len(batch), "elapsed": time.time() - start})
    raise

def complex_function():
    validated = schema.validate(data={"users": [{"name": n, "age": a} for n, a in zip(names, ages)], "config": {"strict": True, "mode": "batch"}}, options={"raise_on_error": True, "collect_errors": False})

    if (isinstance(node, ast.FunctionDef) or isinstance(node, ast.AsyncFunctionDef)) and hasattr(node, "returns") and node.returns is not None:
    ...
    result.update({"processed": True, "output": format_output(data=result["raw"], template=load_template(name="default", version=2), options={"indent": 4, "sort_keys": True})})

    return {"items": sorted([{"id": item.id, "name": item.name, "score": calculate_score(item, weights={"relevance": 0.6, "freshness": 0.3, "popularity": 0.1})} for item in filtered_items], key=lambda x: x["score"], reverse=True), "total": len(filtered_items)}

class ComplexProcessor(BaseProcessor, CacheMixin):
    """Processor."""
    default_config: dict = {"timeout": 30, "retries": 3}
    def process(self, items: list[dict[str, Any]], callback: Callable[[str, dict], tuple[bool, str | None]] | None=None) -> dict[str, list[tuple[str, int]]]:
        results = [{"key": k, "values": [v for v in item[k] if v > self.threshold]} for item in items for k in item if isinstance(item[k], list)]

        if any(len(r["values"]) > self.max_items for r in results) or all(r["values"] == [] for r in results):
...
schema_merge = base_schema | {"required": ["name", "email", "age"], "properties": {"name": {"type": "string", "minLength": 1}, "email": {"type": "string", "format": "email"}, "age": {"type": "integer", "minimum": 0}}}

updated_config = default_config | user_config | {"override": True, "timestamp": time.time()}

class Handler:
    """Handler."""
    def handle(self, event: str, *args, callback: Callable[..., None] | None = None, **kwargs) -> None:
        pass
    def register(self, name: str, handler: Callable[..., Any], *, priority: int = 0, **metadata) -> "Handler":
        pass
...
merged_perms = base_permissions | role_permissions | {"admin": user.is_superuser, "audit": user.has_perm("audit")}

full_config = default_settings | environment_overrides | {"debug": os.environ.get("DEBUG", "false").lower() == "true", "version": __version__}
...
def test_assert_boolop():
    assert "NonExistent" in r['error']['message'] or "resource" in r['error']['message'].lower()
    assert result.get("error") is not None and result['error']['error_type'] == "locality_incompatibility"
    assert a or b or c or d
    assert short or x
    assert some_long_variable_name in some_container and other_long_variable_name in other_container
    assert (
        already_expanded_correctly
        or second_condition
    )
    assert condition_one and condition_two and condition_three and condition_four
    assert func_call(arg1, arg2, kwarg="value") or other_func(x, y) or fallback
    assert isinstance(result, dict) and "key" in result and result['key'] is not None

def test_assert_with_structures():
    assert result == {"status": "ok", "data": [1, 2, 3]} or result == {"status": "pending", "data": []}
    assert response.json() == {"users": [{"id": 1, "name": "Alice"}, {"id": 2, "name": "Bob"}], "total": 2}
    assert sorted(items) == ["alpha", "beta", "gamma", "delta"] and len(items) == 4
    assert config == {"host": "localhost", "port": 8080, "debug": True, "options": {"timeout": 30, "retries": 3}}
    assert errors == [] and warnings == [] and result is not None
    assert output in [{"type": "success"}, {"type": "partial"}] or output.get("override") is True
    assert all(isinstance(item, dict) and "id" in item for item in results) and len(results) > 0
