...
class PyDocstringSpaceTokenizer(Tokenizer):
    ...
    def _find_signature_end_line(
        ...
        path_lookup2: Dict[pathlib.Path, List[Dict[str, str]]] = {}
        path_lookup3:Dict[pathlib.Path, List[Dict[str, Dict[str, int]]]] = {}
    ...
    def _is_import_line(self, line: str) -> bool:
        ...
        thing =  (
            ...
            and (len(stripped) == 6 or stripped[6] in " \n")
        ) or (
            stripped.startswith("yield")
            and (len(stripped) == 5 or stripped[5] in " \n")
        )

        with_long_func = self._find_signature_end_line(["sdfkj", "sdfdf"], 500) and (len(stripped) == 6 or stripped[6] in " \n")
        ...
        return (stripped.startswith("import ")
            or stripped.startswith("from ")
            or stripped.startswith(")")
            or (stripped.startswith("#") and False))
    ...
    def another_format(self, token: str) -> str:
        ...
        if module is None or items is None or len(items) <= 1:
...
my_thing = [
    {
        ...
        "formatters": [PyImportSeparatorFormatter(
